...
export interface QQRoomState {
  ...
  /** 2026-05-17 (P0 Auto-Flow): Liste der Phase-Indizes nach denen schon ein
   *  roundPause-CG getriggert wurde. Wolf-Spec: nach JEDER Non-Final-Runde
   *  ein CG. Reset bei Game-Start. */
...
export function qqStartTieBreaker(room: QQRoomState, durationSec?: number): void {
  ...
  const isReroll = room.phase === 'TIEBREAKER_QUESTION';
  if (room.phase !== 'GAME_OVER' && !isReroll) {
    throw new QQError('WRONG_PHASE', 'Stechen nur am Spielende.');
  }
  ...
  room.phase = 'TIEBREAKER_QUESTION';
  room.lastActivityAt = Date.now();
}
...
/**
 * 2026-07-04: Stechen abbrechen → zurueck zu GAME_OVER (Mod loest ggf. manuell
 * via qqResolveTieBreaker auf). Verwirft die Stechfrage.
 */
export function qqCancelTieBreaker(room: QQRoomState): void {
  if (room.phase !== 'TIEBREAKER_QUESTION') return;
  (room as any).tieBreaker = null;
...
export function qqComebackStealAfterOne(room: QQRoomState): void {
  ...
  // 2026-05-24 (Wolf-Live-Test): KEINE Mod-Pause mehr — analog zum normalen
  // Placement-Flow (Team A setzt → Team B sofort dran). Bei verbleibenden
  // Steals desselben Teams: Leader-Recompute inline + pendingAction setzen.
  // Bei currentStealerRemaining=0: direkt zum naechsten Team in der Queue
  // springen (qqComebackStealStartNext). qqComebackStealStartNext beendet
  // automatisch das Comeback wenn keine Teams mehr in der Queue sind.
  if (hl.currentStealerRemaining > 0) {
    ...
    // Team ist fertig — naechstes Team in der Queue (oder Comeback-Ende).
    qqComebackStealStartNext(room);
  }
  room.lastActivityAt = Date.now();
}
...
export function qqNextQuestion(room: QQRoomState): void {
  ...
  // Nach JEDER Runde (außer der letzten = Final), wenn CG aktiv und noch nicht
  // für diesen Phase-Index getriggert: CG-Phase einschieben. Nach Sieger-
  // Aktion ruft Mod-Space qqNextQuestion erneut → Flag ist gesetzt → normaler
  ...
  if (
    ...
    const currentPhase = room.gamePhaseIndex;
    const phasesPlayed = Array.isArray(room.cozyGamesPlayedAfterPhases)
      ? room.cozyGamesPlayedAfterPhases : [];
    // (a) Nach Non-Final-Runde: roundPause-Slot — vor Phase-Wechsel oder Comeback.
    if (currentPhase < room.totalPhases && !phasesPlayed.includes(currentPhase)) {
      qqStopTimer(room);
      qqFlushQuestionToHistory(room);
...
// ── buildQQStateUpdate liegt seit 2026-08-30 in qqStateUpdate.ts ─────────────
// Die Durchreiche bleibt, damit KEINE Import-Zeile im Projekt sich aendert.
// Begruendung und die Regel gegen Kreis-Unfaelle stehen im Kopf jener Datei.
export { buildQQStateUpdate } from './qqStateUpdate';
...
// ─────────────────────────────────────────────────────────────────────────────
// Final-Wager-Mechanik (Wolf-Idee 2026-05-09): vor der letzten Spiel-Phase
// dürfen Teams Felder als Wetten setzen — auf andere Teams oder auf sich selbst.
// Cap: max floor(eigene_Felder / 2). Bonus pro Bet auf Sieger-Team der Final-
// Runde = +1 Bonus-Coin im End-Score. Bets auf Verlierer-Teams = die Cells
// werden vom Brett entfernt. Cells werden NICHT visuell konvertiert während
// der Bet-Phase (bleiben am Brett); Resolution passiert nach der letzten Frage
// der Final-Phase mit dramatischer Score-Cascade.
// ─────────────────────────────────────────────────────────────────────────────

import type {
  QQFinalBet, QQFinalBetResolution,
} from '../../../shared/quarterQuizTypes';

/** Helper: zählt aktuelle Felder pro Team (für Bet-Cap). */
function countCellsByTeam(room: QQRoomState): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const id of Object.keys(room.teams)) counts[id] = 0;
  for (const row of room.grid) {
    for (const cell of row) {
      if (cell.ownerId && counts[cell.ownerId] !== undefined) counts[cell.ownerId]++;
    }
  }
  return counts;
}
...
export function qqStartFinalBetting(room: QQRoomState): void {
  // Bets dürfen nur einmal pro Spiel laufen — Phase-Wechsel erlaubt aus
  // QUESTION_REVEAL/PLACEMENT/PHASE_INTRO der vorletzten Phase.
...
export function qqFlushPendingStacks(room: QQRoomState): void {
  ...
  // 2026-05-25 v4 (Wolf 'grid zu voll/unuebersichtlich, max 1 stamp pro cell'):
  // LRU-Spread — jeder Stamp landet auf der Cell mit den WENIGSTEN bisherigen
  // Stamps (= bevorzugt unmarkierte Cells). Damit verteilen sich Stamps auf
  // mehrere Cells, jede Cell zeigt nur Team-Avatar + 1 Stamp (diagonal).
  const N = room.gridSize;
  const ownCells: Array<{ row: number; col: number }> = [];
  for (let r = 0; r < N; r++) {
    for (let c = 0; c < N; c++) {
  ...
  // Cluster-Centroid an Index 0 fuer stabile Priorisierung beim ersten Stamp.
