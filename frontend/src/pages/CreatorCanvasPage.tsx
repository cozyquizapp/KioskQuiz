...
export default function CreatorCanvasPage() {
  ...
  const [themeColor, setThemeColor] = useState('#7a5bff')
  const [bg, setBg] = useState('')
  const [logo, setLogo] = useState('')
  const [font, setFont] = useState('Geist')
  const [intro, setIntro] = useState('start')
  const [rules, setRules] = useState('start')
  const [animation, setAnimation] = useState('Slide')
  const [introText, setIntroText] = useState('Willkommen zum Cozy Kiosk Quiz!')
  const [rulesText, setRulesText] = useState('Regeln kurz erklaeren')
