...
function ModeratorPage(): React.ReactElement {
  ...
  const renderStageShortcuts = () => {
    ...
    if (isScoreboardState && askedCount >= totalQuestions) {
      ctas.push(
        <button
          key="cta-awards"
    ...
    {toast && (
        <div
          style={{
            ...
            padding: '10px 14px',
            borderRadius: 12,
            background: toast.toLowerCase().includes('fehl') || toast.toLowerCase().includes('error') || toast.toLowerCase().includes('nicht') ? '#dc2626' : 'rgba(15,23,42,0.9)',
            color: '#ffffff',
