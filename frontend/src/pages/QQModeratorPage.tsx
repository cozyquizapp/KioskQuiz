...
import { QQSetupFlow } from './QQSetupFlow';
...
export default function QQModeratorPage({ testMode = false }: { testMode?: boolean } = {}) {
  ...
  // 2026-07-19 (Wolf 'Format-Wahl wird zum Wizard, ersetzt das Panel'): Es gibt
  // nur noch EIN Gate — setupDone. !setupDone → gefuehrter QQSetupFlow-Wizard
  // (Format→Look→...→Bereit), setupDone → schlankes Cockpit. Die frueheren States
  // editFormat/showAllSettings (zweite Buehne + separates Panel) sind entfallen.
  ...
  return (
    ...
    <div className="qq-mod-shell" style={page}>
          ...
          <div style={{ minHeight: 'calc(100dvh - 124px)', display: 'flex', flexDirection: 'column', justifyContent: hasTeams ? 'flex-start' : 'center', paddingTop: hasTeams ? 18 : 0, gap: 'clamp(8px, 1.8vh, 18px)', maxWidth: 860, margin: '0 auto', width: '100%' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, justifyContent: 'center', alignItems: 'stretch' }}>
              ...
              <div style={{ ...card, flex: '1 1 380px', maxWidth: 560, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {/* 2026-07-19 (Wolf 'Wizard ersetzt das Panel'): Format/Look/Ort/
                    Runden/Timer/Sprache werden im gefuehrten Setup-Wizard gesetzt —
                    hier nur noch eine kompakte Read-only-Zusammenfassung. Aendern via
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                  {[
                    ...
                    <span key={chip.k} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 11px', borderRadius: 999, fontSize: 12, fontWeight: 800, background: 'rgba(148,163,184,0.08)', border: '1px solid rgba(148,163,184,0.2)', color: '#cbd5e1' }}>
                      <span style={{ fontSize: 9, fontWeight: 900, color: '#64748b', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{chip.k}</span>
                      {chip.v}
                    </span>
                  ))}
                  <button onClick={() => setSetupDone(false)} title="Setup-Wizard öffnen (Format, Look, Runden, Timer, Sprache, Extras)"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 999, fontSize: 12, fontWeight: 900, cursor: 'pointer', fontFamily: 'inherit', background: `${accent}1e`, border: `1px solid ${accent}55`, color: '#fff' }}>
                    ⚙ Ändern
                  </button>
                </div>
              </div>
      ...
      {/* 2026-07-19 (Wolf 'Format-Wahl wird zum Wizard, ersetzt das Panel'): der
          gefuehrte 7-Schritt-Setup-Flow IST jetzt die einzige Einrichtungs-Flaeche
          (Format -> Look -> Fragensatz -> Runden -> Timer/Sprache -> Extras ->
          Bereit). Gate = !setupDone; „Ins Cockpit" setzt setupDone. Danach das
          schlanke Cockpit (Teams + Start). Loest Format-Landing + Settings-Panel ab. */}
      {joined && s && s.phase === 'LOBBY' && !setupDone && (
        <QQSetupFlow
          s={s}
          drafts={drafts}
