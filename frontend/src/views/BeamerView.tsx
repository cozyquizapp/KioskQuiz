...
const BeamerView = ({ roomCode }: BeamerProps) => {
  ...
  // Trigger confetti when showcasePhase becomes FINAL_CARDS (Fotosprint)
  useEffect(() => {
    if (showcasePhase === 'FINAL_CARDS' && gameState === 'BLITZ_CATEGORY_SHOWCASE') {
      if (!confettiRef.current && typeof document !== 'undefined') {
        confettiRef.current = createConfetti(document.body);
      }
      confettiRef.current?.explosion(150);
    }
  }, [showcasePhase, gameState]);
  ...
  // Trigger confetti when rundlaufShowcasePhase becomes FINAL_CARDS (K.O.-Rallye)
  useEffect(() => {
    if (rundlaufShowcasePhase === 'FINAL_CARDS' && gameState === 'RUNDLAUF_CATEGORY_SHOWCASE') {
      if (!confettiRef.current && typeof document !== 'undefined') {
        confettiRef.current = createConfetti(document.body);
      }
      confettiRef.current?.explosion(150);
    }
  }, [rundlaufShowcasePhase, gameState]);
...
useEffect(() => {
  ...
  const renderCozyIntroContent = (): JSX.Element => {
    ...
    const lobbyTitle = isBoth ? (
      ...
      <div style={{ color: '#f05fb2', fontFamily: 'var(--font-game)', fontWeight: 900, fontSize: 'clamp(24px, 6vw, 42px)', textAlign: 'center', textTransform: 'uppercase' }}>
        {isEn ? 'WELCOME TO COZY WOLF QUIZ' : 'WILLKOMMEN ZUM COZY WOLF QUIZ'}
      </div>
    );

    const waitingLabel = isBoth ? (
      <BilingualLabel
      ...
      <div style={{ color: '#f1f5f9', fontFamily: 'var(--font-game)', fontWeight: 700, fontSize: 'clamp(16px, 4vw, 24px)', textAlign: 'center' }}>
        {isEn ? 'WAITING FOR TEAMS TO JOIN' : 'WARTEN AUF TEAMS'}
      </div>
    );

    const scanToJoinLabel = isBoth ? (
      <BilingualLabel
        en="SCAN TO JOIN"
      ...
      <div style={{ color: '#f05fb2', fontFamily: 'var(--font-game)', fontWeight: 700, fontSize: 'clamp(16px, 4vw, 24px)', textAlign: 'center', textTransform: 'uppercase' }}>
        {isEn ? 'SCAN TO JOIN' : 'SCANNEN ZUM BEITRETEN'}
      </div>
    );
            ...
            <div className="cozyLobbyTeamsList">
              {sortedTeams.length === 0 ? (
                ...
                sortedTeams.map((team) => {
                  ...
                  const statusText = isBoth ? undefined : (isEn ? statusEn : statusDe);
                    ...
                    <div className="cozyLobbyTeamRow" key={team.id}>
                      ...
                      <div className="cozyLobbyTeamStatus">
                        {isBoth ? (
                          ...
                          <span style={{ fontSize: 'clamp(11px, 2vw, 14px)', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: isConnected ? '#22c55e' : '#ef4444' }}>
                            {statusText}
                          </span>
                        )}
                      </div>
  ...
  const renderCozyBlitzContent = (): JSX.Element | null => {
    ...
    if (phase === 'ROUND_INTRO') {
      ...
      const currentSetIndex = (blitz?.setIndex ?? 0) + 1;
          ...
          <div className="beamer-intro-card">
            ...
            {countdownNumber !== null && (
              <div style={{
                ...
                marginTop: '30px',
                animation: 'countdown-pop 0.8s ease-in-out infinite'
              }}>
                {countdownNumber}
              </div>
            )}
  ...
  const renderCozyScene = () => {
    ...
    const renderRundlaufFrame = () => {
                ...
                <div className="blitz-pool-grid" style={{ position: 'relative' }}>
                  {displayPool.map((entry, entryIdx) => {
                    ...
                    const cardClasses = [
                      ...
                      isBanned ? 'banned' : '',
                      isPicked ? 'picked' : '',
                      (isSlot1 || isSlot2) ? 'slot-highlight' : ''
                    ].filter(Boolean).join(' ');
          ...
          >
            <div className="blitz-stack" style={{ padding: '20px 24px', animation: 'zoomIn 0.8s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>
              <div style={{
                ...
                {/* Random categories — all appear almost simultaneously */}
                {randomCategories.map((cat, idx) => (
                  <div
      ...
      if (gameState === 'RUNDLAUF_ROUND_INTRO') {
        ...
        const totalRounds = Math.max(1, (rundlauf?.selected ?? []).length);
        return (
          <BeamerFrame
          ...
          >
            <div className="beamer-intro-card">
              ...
              <div style={{
                ...
                {currentCategoryTitle || 'Kategorie'}
              </div>
              {/* Round badge */}
              <div style={{
                fontSize: '18px',
                ...
                Runde {roundLabel}/{totalRounds}
              </div>
              <p style={{ fontSize: '16px', opacity: 0.85, marginTop: '10px' }}>
                {language === 'de'
