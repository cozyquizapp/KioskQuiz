...
const RENNEN = /Die Siegerehrung|The awards/;
for (let i = 0; i < 30; i++) {
...
for (let i = 0; i < 10; i++) { await h.emit("qq:nextQuestion"); await sleep(4200); }
...
await blatt(Array.from({ length: 24 }, (_, i) => i * 1900), 6, 300, '.shots/RENNEN.png');
await b.schliessen?.();
process.exit(0);
